...
- GitHub CLI (`gh`) — самый простой способ аутентификации. Избегай ручной работы с SSH-ключами.
- Флаг `--private` важен: код не должен быть публичным в процессе обучения.
- Если репо с таким именем уже есть — `gh` вернёт ошибку, помоги выбрать другое имя.
- `gh repo view --web` — чище, чем конструировать URL руками.
- Если brew недоступен — отправь на cli.github.com за инсталлятором.
- Next.js автоматически генерит `.gitignore`; `node_modules` не коммитим.
