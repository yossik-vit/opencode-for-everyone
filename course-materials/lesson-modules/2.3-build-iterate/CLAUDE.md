...
[Если установка не удалась: "Хочешь, разберёмся? Частые причины: нет Node.js, устаревший npm, проблемы с сетью — проверим через `node --version` и `npm --version`. Или пропустим настройку?"]
...
<!-- EXPECTED: Выглядит хорошо / [Вопросы] -->
...
[Если сервер не запустился: "Хочешь, разберёмся? Проверим вывод терминала на ошибки. Частые причины: порт 3000 занят, не установлены зависимости. Или пропустим?"]
...
<!-- EXPECTED: Да работает! / [Что-то не так] -->
...
Трюк: сделай скриншот, перетащи его в панель AI-агента (или вставь через стандартный `Cmd/Ctrl+V`), скажи, что изменить.
...
STOP: Сделай скриншот викторины, перетащи его в панель агента (или вставь через Cmd/Ctrl+V), скажи, что изменить.

<!-- EXPECTED: [Вставляет скриншот и описывает изменение] -->
...
<!-- EXPECTED: [Больше изменений / Доволен этим] -->
...
<!-- EXPECTED: Давай GitHub -->

---

## Важные заметки для AI-агента (проводящего урок)

- `npm run dev` — запускай в фоне, не блокируй сессию.
- `localhost:3000` — порт Next.js по умолчанию.
- Скриншоты: смотри внимательно, предлагай конкретные изменения, а не абстрактные.
- Картинки — перетаскивание в панель или обычный `Cmd/Ctrl+V`.
- Итерации — сфокусированные. Не впихивай 10 изменений в одно сообщение.
- Баги — сначала лечим, потом визуально полируем.
- Plan mode (`Shift+Tab`) — реальная функция, используй её для больших шагов.
