...
[Если файлы не открылись: "Хочешь, разберёмся? Проверим, что файлы созданы через `ls quiz-project/style-preview*.html`. Или пропустим и посмотрим позже?"]

Посмотри на предпросмотры. Каждая вкладка — разный стиль.

STOP: Какой стиль нравится? Можешь выбрать один или смешать.

<!-- EXPECTED: [Отвечает с предпочтением] -->
...
[Если изображения не скачались: "Хочешь, разберёмся? Возможно, URL устарели — попробуем другие источники. Или пропустим изображения и добавим позже?"]
...
<!-- EXPECTED: [Одобряет или предлагает изменения] -->
...
[Если файл не создался: "Хочешь, разберёмся? Проверим права и директорию через `ls quiz-project/`. Или пропустим?"]

Готово! Открой REQUIREMENTS.md. Это спецификация для строительства.

STOP: Всё выглядит правильно?

<!-- EXPECTED: Да / [Корректировки] -->
...
<!-- EXPECTED: Давай строить -->

---

## Важные заметки для AI-агента (проводящего урок)

- `AskUserQuestion` — только для начального набора вопросов (личности/стиль).
- Предпросмотры стилей — визуально различимые, чтобы выбор был очевидным.
- Изображения — Unsplash, проверяй URL перед скачиванием.
- `REQUIREMENTS.md` критически важен для урока 2.3 — без него строить нечего.
- Смешивание стилей — уточняй, какие именно элементы объединить.
- Количество вопросов: больше пар личностей → больше вопросов.
